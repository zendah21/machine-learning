...
def load_features(folder_path, n_components=2000):
    all_features = []
    all_labels = []
    ...
    for filename in os.listdir(folder_path):
        if filename.lower().endswith('.pkl'):
            ...
            label = os.path.splitext(filename)[0]  # Extract label from filename without an extension
...
def save_model(model, filename='knn_model_multiclass.joblib'):
